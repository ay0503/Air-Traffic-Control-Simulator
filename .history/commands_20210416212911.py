...
def divideCommand(cmd):
    parameters = dict()
    parameters['callsign'] = findCallsign(cmd)
    parameters['alt'] = findAltCommand(cmd)
    parameters['hdg'] = findHdgCommand(cmd)
    parameters['spd'] = findCallsign(cmd)
    return parameters

def exectuteCommand(aircraft, cmd):
    parameters = divideCommand(cmd)

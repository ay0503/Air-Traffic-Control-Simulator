...
def drawAircraft(app, canvas, plane):
    ...
    canvas.create_text(x - hdgVector(plane.hdg, plane.spd / 16)[0], y + hdgVector(plane.hdg, plane.spd / 16)[1], text = info, 
...
def drawSidebar(app, canvas):
    ...
    canvas.create_rectangle(app.boardWidth, 0, app.width, app.height,
                            fill = app.color)
    canvas.create_text((app.boardWidth + app.width) / 2, 20, text = 'Flights',
                        font = "Arial 14 bold")
    for row in range(min(len(app.aircrafts), 5)):
        ...
        canvas.create_rectangle(app.boardWidth + margin, 30 + margin + row * 50,
                                app.width - margin, 30 + margin + (row + 1) * 50,
                                outline = 'black', )
        canvas.create_text((app.boardWidth + app.width) / 2, 30 + margin + (row + 0.5) * 50,

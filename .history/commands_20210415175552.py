...
def findAltCommand(cmd):
    words = cmd.split(" ")
    if  "climb" in words:
        altComm = words[words.index("climb"):]
        return findAltitude(altComm)
    elif "descend" in words:
        altComm = words[words.index("descend"):]
        return findAltitude(altComm)
    return None
...
def findHdgCommand(cmd):
    words = cmd.split(" ")
    if "heading" in words:
        hdgComm = words[words.index('heading'):]
        print(findHeading(hdgComm))
        return findHeading(hdgComm)
    elif "turn" in words:
        hdgComm = words[words.index('turn'):]
        return findHeading(hdgComm)
    return None

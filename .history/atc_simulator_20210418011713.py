...
def appStarted(app):
    ...
    app.airport = Airport("KLAX", [[0, -6, 83], [0, -11, 83], [0, +6, 83], [0, +11, 83]], 'F')
    ...
    app.displayLength = min(len(app.flights), app.height // 16)
    app.display = app.flights[app.index:app.index + app.displayLength]
...
def keyPressed(app, event):
    ...
    if event.key in string.printable:
        ...
        if app.display != app.flights[:]:
            app.index += 1
            app.display = app.flights[app.index:app.index + app.displayLength]
    elif event.key == "Down":
        if app.display != app.flights[len(app.flights) - min(len(app.flights), app.height // 18):]:
            app.index -= 1
            app.display = app.flights[app.index:app.index + app.displayLength]
...
def sizeChanged(app):
    ...
    app.displayLength = min(len(app.flights), app.height // 18)
    print(app.displayLength)

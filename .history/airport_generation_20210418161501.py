...
airport = KLAX = Airport("KLAX", [[0, -6, 83], [0, -11, 83], [0, +6, 83], [0, +11, 83]], 'F')
# airport parameters
...
def newLength(airport):
    if ord(airport.size) < ord("C"):
        return random.randrange(4000, 6000)
    else: 
        diff = ord(airport.size) - ord("A")
        return random.rangerange(6000 + diff * 1000, 7000 + diff * 1000)

    

def newRwyPos(aiport):
    pass

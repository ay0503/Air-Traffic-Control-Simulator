...
def drawSidebar(app, canvas):
    ...
    canvas.create_text((app.mapWidth + app.width) / 2, 15, text = 'Flights',
                        font = "Arial 14 bold")
    # timer
    canvas.create_rectangle(app.mapWidth + margin, app.height - (50 + margin),
                                    app.width - margin, app.height - margin,
                                    outline = 'black', width = 2, fill = app.color)
    canvas.create_text((app.mapWidth + app.width) / 2, app.height - margin - 25, 
                    text = f'Airport: {app.airport.code} \nTimer: {int(app.timer // 60)}:{int((app.timer % 60))}', font = "Arial 21 bold")
    # draw flight sticks

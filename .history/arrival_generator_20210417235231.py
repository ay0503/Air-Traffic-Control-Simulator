...
airport = Airport("KLAX", [[0, -6, 83], [0, -11, 83], [0, +6, 83], [0, +11, 83]], 'F')
...
def compareSize(s1):
    return ord(s1) > ord(airport.size)
...
def newType(aircrafts, airport):
    ...
    print(len(result))
    result = filter(compareSize, result, airport.size)
    print(len(result))
...
def createArrival(width, height, airport):
    ...
    return Flight(newCallsign(), type, pos, newHeading(pos, width, height), 
                    newSpeed(), newAltitude(), 0, start, end)

print(newType(aircrafts, KLAX))

...
def keyPressed(app, event):
    print(0 < event.x < app.mapWidth and app.mapHeight < event.y < app.height)
...
def drawSidebar(app, canvas):
    ...
    if len(app.flights) > 0:
        for row in range(len(app.display)):
            ...
            print(callsigns, flight.callsign)
            # object recognition not working on macOS 11.2.3
            callsigns = list(map(lambda x: x.callsign, app.flights))
            info = f"{callsigns.index(flight.callsign) + 1}. {flight.callsign}, {flight.type.code}, {flight.hdg}°, {flight.spd}kt, \n{int(flight.alt)}ft,  {flight.vs}ft/m,  {flight.start} - {flight.end}"

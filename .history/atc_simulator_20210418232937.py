...
def drawAirport(app, canvas):
    ...
    for runway in app.airport.runways:
        ...
        # draw ILS range
        canvas.create_line(p1, p2, fill = app.color)
        canvas.create_line(p1, p3, fill = app.color)
        canvas.create_line(p3, p2, fill = app.color)
        canvas.create_line(runway.pos, runway.beacon, fill = app.color, dash = (5, 1))
        # draw runway

...
class Arrival(Flight):
    ...
    def intercept_ILS(self, runway):
        if abs(self.hdg - runway.hdg) < 30:
            self.change_heading(runway.hdg)
...
class Airport(object):
    ...
    def create_waypoints(self, width, height):
        ...
        waypoints = []
        for i in range(6 * (ord(self.size) - ord("A"))):
            x = y = 0
            while not (self.pos[0] - 50 < x < self.pos[0] + 50 and self.pos[0] - 50 < y < self.pos[0] + 50):
                x, y = random.randrange(100, width - 100), random.randrange(50, height - 50)
            waypoints.append(Waypoint(name, [x, y]))

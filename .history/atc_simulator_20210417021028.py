...
def appStarted(app):
    ...
    app.aircrafts = []
    for k in range(2):
        app.aircrafts.append(createArrival(app.boardWidth, app.boardHeight, app.airport))
...
def timerFired(app):
    ...
    for aircraft in app.aircrafts:
        ...
        if not aircraft.checkOnGrid(app.boardWidth, app.boardHeight):
            app.aircrafts.remove(aircraft)
        
    # TODO check distancing
    print(keepDistance(app.aircrafts))
...
def drawAircraft(app, canvas, plane):
    ...
    # safety ring
    canvas.create_oval(x - r, y - r, x + r, y + r, outline = app.color)

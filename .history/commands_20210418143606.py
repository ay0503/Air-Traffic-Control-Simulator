...
def findCallsign(cmd, aircrafts):
    ...
    for aircraft in aircrafts:
        ...
        print(aircraft.callsign[0] + ' ' + aircraft.callsign[1])
        if word[0:1] == aircraft.callsign[0] + ' ' + aircraft.callsign[1]:
            return aircraft.callsign
        # verbal case (spaces)
        if word[:1] == aircraft.fltno():
            return aircraft.callsign
...
def testDivideCommand():
    ...
    print(divideCommand("Korean 123 turn right heading 350", flights))
    print("Passed")

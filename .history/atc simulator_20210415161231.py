...
def drawSidebar(app, canvas):
    ...
    for row in range(len(app.aircrafts)):
        ...
        canvas.create_rectangle(app.boardWidth + margin, 40 + margin + row * 50,
                                app.width - margin, 40 + margin + (row + 1) * 50,
                                outline = 'black', )
        canvas.create_text((app.boardWidth + app.width) / 2, 40 + margin + (row + 0.5) * 50,
